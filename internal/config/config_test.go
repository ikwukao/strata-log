...
func TestLoadDefaults(t *testing.T) {
	...
	if cfg.Storage.Path != "strata-log.db" {
		t.Fatalf("expected default storage path strata-log.db, got %q", cfg.Storage.Path)
	}

	if cfg.Buffer.Capacity != 10_000 {
		t.Fatalf("expected default buffer capacity 10000, got %d", cfg.Buffer.Capacity)
	}

	if cfg.Batcher.MaxSize != 100 {
		t.Fatalf("expected default batch size 100, got %d", cfg.Batcher.MaxSize)
	}
...
func TestLoadFromEnvironment(t *testing.T) {
	...
	if cfg.Storage.Path != "/tmp/strata-log.db" {
		t.Fatalf("expected custom storage path, got %q", cfg.Storage.Path)
	}

	if cfg.Buffer.Capacity != 5000 {
		t.Fatalf("expected buffer capacity 5000, got %d", cfg.Buffer.Capacity)
	}

	if cfg.Batcher.MaxSize != 250 {
		t.Fatalf("expected batch size 250, got %d", cfg.Batcher.MaxSize)
	}

	if cfg.Batcher.FlushPeriod != time.Second {
		t.Fatalf("expected flush period 1s, got %s", cfg.Batcher.FlushPeriod)
	}

	if cfg.Shutdown.Timeout != 20*time.Second {
		t.Fatalf("expected shutdown timeout 20s, got %s", cfg.Shutdown.Timeout)
	}
}
